...
class DomainTest {
  ...
  void addRecord_adds_a_new_dns_record() {
    ...
    domain.addRecord(Type.A, "127.0.0.1");
    domain.addRecord(Type.A, "127.0.0.2");
  ...
  void addRecord_fails_when_record_already_exists() {
    final var domain = Domain.of("example.com");
    domain.addRecord(Type.A, "127.0.0.1");
    Assertions.assertThrows(RecordAlreadyExistsException.class, () -> domain.addRecord(Type.A, "127.0.0.1"));
  }
  ...
  void removeRecord_removes_a_dns_record() {
    ...
    Assertions.assertEquals(new Record(Type.A, "127.0.0.2"), domain.records().iterator().next());
  }
